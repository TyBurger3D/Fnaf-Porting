...
using FNAFPorting.Models.Levels;
...
public partial class SupabaseService : ObservableObject, IService
{
    ...
    public SupabaseService(APIService api)
    {
        ...
        {
            var auth = await Api.FNAFPorting.AuthInfo();
            if (auth is null)
            {
            ...
            Client.Auth.AddStateChangedListener(async (client, state) =>
            {
                if (client.CurrentSession is { } session)
                {
                    ...
                    PersistSession(sessionInfo.ToEncryptedString());
                }
            ...
            if (AppSettings.Account.SessionInfoEncrypted is { } encryptedSession)
                sessionInfo = UserSessionInfo.FromEncryptedString(encryptedSession);

            sessionInfo ??= SharedAuthSession.TryLoadFromFortnitePorting();
    ...
    public async Task SetSession(UserSessionInfo sessionInfo)
    {
        try
        {
            if (await Client.Auth.SetSession(sessionInfo.AccessToken, sessionInfo.RefreshToken) is { } session)
            {
                ...
                PersistSession(refreshedSessionInfo.ToEncryptedString());
            }
        }
    ...
    private static void PersistSession(string? encryptedSession)
    {
        AppSettings.Account.SessionInfoEncrypted = encryptedSession;
        SharedAuthSession.SyncEncryptedSession(encryptedSession);
    }
    ...
    {
        await Api.FNAFPorting.PostExports(objectPaths);
    }
    ...
    private async Task PostLogin()
    {
        ...
        await Api.FNAFPorting.PostLogin();
        _postedLogin = true;
    }
    ...
    public async Task<UserInfoResponse?> GetUserAsync(string? id)
    {
        ...
        var userInfo = await Api.FNAFPorting.UserInfo(id);
