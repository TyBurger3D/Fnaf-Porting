...
public class DependencyService : IService
{
    ...
    private void EnsureBlenderExtensions()
    {
        ...
        var assets = AssetLoader.GetAssets(new Uri("avares://FNAFPorting.Plugins/Blender"), null);
        WriteAssets(assets, App.PluginsFolder.FullName);
        var assetsUeformat = AssetLoader.GetAssets(new Uri("avares://FNAFPorting.Plugins/UEFormat/Blender/io_scene_ueformat"), null);
        WriteAssets(assetsUeformat, Path.Combine(App.PluginsFolder.FullName, "Blender", "fnaf_porting", "ueformat"), 36);
    }
    ...
    private void EnsureUnrealPlugins()
    {
        ...
        var assets = AssetLoader.GetAssets(new Uri("avares://FNAFPorting.Plugins/Unreal"), null);
        WriteAssets(assets, App.PluginsFolder.FullName);
        var assetsUeformat = AssetLoader.GetAssets(new Uri("avares://FNAFPorting.Plugins/UEFormat/Unreal/UEFormat"), null);
