    ...
    public bool Match(string filter, bool useRegex = false)
    {
        if (useRegex)
        {
            return this switch
            {
                AssetItem assetItem => Regex.IsMatch(assetItem.CreationData.DisplayName, filter)
                                                    || Regex.IsMatch(assetItem.CreationData.DisplayName, filter)
                                                    || Regex.IsMatch(assetItem.CreationData.ID, filter)
        ...
        return this switch
        {
            AssetItem assetItem => MiscExtensions.Filter(assetItem.CreationData.DisplayName, filter)
                                   || MiscExtensions.Filter(assetItem.CreationData.DisplayName, filter)
                                   || MiscExtensions.Filter(assetItem.CreationData.ID, filter)
