...
using SkiaSharp;
using AssetLoader = FNAFPorting.Models.Assets.Loading.AssetLoader;
...
public partial class AssetStyleInfo : ObservableObject
{
    ...
    public AssetStyleInfo(string channelName, FStructFallback[] styles, Bitmap fallbackPreviewImage, bool addDefault = false)
    {
        ...
        foreach (var style in styles)
        {
            ...
            {
                var skinPreviewBitmap = fallbackPreviewImage;
                var stylePreviewPath = "Marvel/Content/Marvel/UI/Textures/Item/Skin/item_skin_" + skinId;
                if (UEParse.Provider.TryLoadPackageObject(stylePreviewPath, out UTexture2D skinPreviewTexture))
                {
                    var previewDecodedTex = skinPreviewTexture.Decode();
                    if (previewDecodedTex != null) skinPreviewBitmap = previewDecodedTex.ToWriteableBitmap();
                }

                StyleDatas.Add(new AssetStyleData(style, skinPreviewBitmap));
                continue;
            }
