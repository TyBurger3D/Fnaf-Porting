...
using FNAFPorting.Models.Article;
...
public partial class HelpViewModel : ViewModelBase
{
    ...
    {
        var response = await Api.FNAFPorting.GetArticles();
            ...
            new DialogButton
            {
                ...
                Action = async () =>
                {
                    ...
                    for (var i = 0; i < newArticle.Sections.Count; i++)
                    {
                        ...
                        var uploaded = await Api.FNAFPorting.UploadArticleImage(bytes, fileName);
                    ...
                    if (!string.IsNullOrEmpty(newArticle.Id))
                        await Api.FNAFPorting.UpdateArticle(newArticle.Id, request);
                    else
                        await Api.FNAFPorting.CreateArticle(request);
        ...
        Info.Dialog($"Are you sure you would like to delete \"{article.Title}\"?",
            ...
            [
                new DialogButton
                {
                    ...
                    Action = async () =>
                    {
                        await Api.FNAFPorting.DeleteArticle(article.Id);
                        await UpdateArticles();
                    }
                }
            ]);
