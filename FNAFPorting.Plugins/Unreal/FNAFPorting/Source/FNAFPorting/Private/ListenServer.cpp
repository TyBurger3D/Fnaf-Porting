...
#include "FNAFPorting/Public/FNAFPorting.h"
...
{
	Thread = FRunnableThread::Create(this, TEXT("FNAFPortingListenServer"));
}
...
bool FListenServer::Init()
{
	...
	Socket = FTcpSocketBuilder(TEXT("RPV4 Listen Socket"))
		.AsBlocking()
		.AsReusable()
...
{
	UE_LOG(LogFNAFPorting, Log, TEXT("Running FP V4 Server at %s"), *Endpoint.ToString())
	...
	while (bIsRunning)
	{
		...
		{
			UE_LOG(LogFNAFPorting, Warning, TEXT("Failed to accept client connection"));
			continue;
		}
        
		UE_LOG(LogFNAFPorting, Log, TEXT("Client connected from %s"), *RemoteAddress->ToString(true));
...
void FListenServer::Stop()
{
	...
	UE_LOG(LogFNAFPorting, Log, TEXT("Shutdown FP V4 Server"))
}
...
void FListenServer::HandleClient(FSocket* ClientSocket)
{
	...
	while (bIsRunning)
	{
		...
		auto ReceivedString = FFNAFPortingUtils::BytesToString(Data);

		switch (Header->Type)
		{
		case EMessageCommandType::Message:
			UE_LOG(LogFNAFPorting, Log, TEXT("%s"), *ReceivedString)
			break;
	...
	UE_LOG(LogFNAFPorting, Log, TEXT("Client disconnected: %s"), *ClientAddress->ToString(true));
