#include "FNAFPorting/Public/Processing/ImportContext.h"
...
#include "Processing/FNAFPortingTexturePipeline.h"
...
#include "Utilities/FNAFPortingUtils.h"
...
void FImportContext::RunExportJson(const FString& Data)
{
	...
	{
		UE_LOG(LogFNAFPorting, Error, TEXT("Unable to deserialize response from FNAFPorting"))
		return;
	}
...
void FImportContext::EnsureDependencies()
{
	if (DefaultMaterial == nullptr)
		DefaultMaterial = Cast<UMaterial>(UEditorAssetLibrary::LoadAsset("/FNAFPorting/Materials/M_MR_Default.M_MR_Default"));
	if (HeroMaterial == nullptr)
		HeroMaterial = Cast<UMaterial>(UEditorAssetLibrary::LoadAsset("/FNAFPorting/Materials/M_MR_Hero.M_MR_Hero"));
	if (HairMaterial == nullptr)
		HairMaterial = Cast<UMaterial>(UEditorAssetLibrary::LoadAsset("/FNAFPorting/Materials/M_MR_Hair.M_MR_Hair"));
	if (EyeMaterial == nullptr)
		EyeMaterial = Cast<UMaterial>(UEditorAssetLibrary::LoadAsset("/FNAFPorting/Materials/M_MR_Eye.M_MR_Eye"));
	if (EyeGlassMaterial == nullptr)
		EyeGlassMaterial = Cast<UMaterial>(UEditorAssetLibrary::LoadAsset("/FNAFPorting/Materials/M_MR_EyeGlass.M_MR_EyeGlass"));
	if (TranslucentMaterial == nullptr)
		TranslucentMaterial = Cast<UMaterial>(UEditorAssetLibrary::LoadAsset("/FNAFPorting/Materials/M_MR_Translucent.M_MR_Translucent"));
	if (RimMaterial == nullptr)
		RimMaterial = Cast<UMaterial>(UEditorAssetLibrary::LoadAsset("/FNAFPorting/Materials/M_MR_Rim.M_MR_Rim"));
	if (LayerMaterial == nullptr)
		LayerMaterial = Cast<UMaterial>(UEditorAssetLibrary::LoadAsset("/FNAFPorting/Materials/M_FP_Layer.M_FP_Layer"));
}
...
void FImportContext::ImportTextureData(const FJsonWrapper& ExportData)
{
	...
	for (const auto& TextureJson : Textures)
	{
		...
		auto* Pipeline = NewObject<UFNAFPortingTexturePipeline>(GetTransientPackage());
...
UMaterialInstanceConstant* FImportContext::ImportMaterial(const FJsonWrapper& MaterialData)
{
	...
	const FString BaseMaterialPath = MaterialData.Get<FString>("BaseMaterialPath");
	
	if (FFNAFPortingUtils::Any<FJsonWrapper>(MaterialData.GetArray("Switches"), [&](const FJsonWrapper& Item)
	{
		return FNames::LayerSwitchNames.Contains(Item.Get<FString>("Name"));
	}) && FFNAFPortingUtils::Any<FJsonWrapper>(MaterialData.GetArray("Textures"), [&](const FJsonWrapper& Item)
	{
		return FNames::LayerTextureNames.Contains(Item.Get<FString>("Name"));
	}))
	{
	...
	if (FFNAFPortingUtils::Any<FString>(FNames::HeroBaseMaterialNames, [&](const FString& Name)
	{
		return BaseMaterialPath.Contains(Name, ESearchCase::IgnoreCase);
	}))
	{
	...
	if (BaseMaterialPath.Contains("Hair", ESearchCase::IgnoreCase))
	{
		TargetMaterial = HairMaterial;
		TargetMappings = FMaterialMappings::Hair;
	}
	
	if (FFNAFPortingUtils::Any<FString>(FNames::EyeBaseMaterialNames, [&](const FString& Name)
	{
		return BaseMaterialPath.Contains(Name, ESearchCase::IgnoreCase);
	}))
	{
	...
	if (FFNAFPortingUtils::Any<FString>(FNames::EyeGlassBaseMaterialNames, [&](const FString& Name)
	{
		return BaseMaterialPath.Contains(Name, ESearchCase::IgnoreCase);
	}))
	{
	...
	if (FFNAFPortingUtils::Any<FString>(FNames::TranslucentBaseMaterialNames, [&](const FString& Name)
	{
		return BaseMaterialPath.Contains(Name, ESearchCase::IgnoreCase);
	}))
	{
	...
	if (BaseMaterialPath.Contains("Rim", ESearchCase::IgnoreCase))
	{
