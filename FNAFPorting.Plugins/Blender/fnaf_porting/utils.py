...
blend_files = ["fnaf_porting_data.blend"]


def _read_meta_version() -> tuple:
    meta_path = os.path.join(os.path.dirname(__file__), "fnaf_porting_meta.json")
    if not os.path.exists(meta_path):
        return tuple(sys.modules["fnaf_porting"].bl_info["version"])
    try:
